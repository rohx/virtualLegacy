...
class ViewController: UIViewController, ARSCNViewDelegate, SCNPhysicsContactDelegate, AVAudioPlayerDelegate {
    ...
    var animationInvertal : TimeInterval = 10
            ...
            if (name?.hasAncestor("Atomo"))!{
                if(!atomHasFollowed){
                    ...
                    if(isBezierTimerActive){
                        bezierTimer = Timer.scheduledTimer(timeInterval: 0, target: self, selector: #selector(ViewController.atomFollow), userInfo: nil, repeats: true)
                ...
                for hit in hitTest{
                    for planeNode in planeNodes{
                        if (hit.node.hasAncestor(planeNode)){
                            ...
                            clearPlanes()
                            //función para limpiar planos 5 segundos después
    ...
    */
    ...
    @objc func atomCheck() {
        ...
        if (!isInScene && checkerTimer != nil){
            //print("atom not in scene, following")
            ...
            //print("atom is in scene, not following")
        }
    }
    ...
    @objc func atomFollow() {
        // To do, change bezier control points, and bezier inc
...
/*
 ...
 print("####### URLS #######")
 var count = 0
 let hueyapanFiles = Bundle.main.urls(forResourcesWithExtension: "dae", subdirectory: "art.scnassets/Hueyapan")
 for (_, file) in (hueyapanFiles?.enumerated())!{
 do{
 let scene = try SCNScene(url: file, options: [:])
 let node = scene.rootNode.childNodes[0]
 self.sceneView.scene.rootNode.addChildNode(node);
 node.worldPosition = SCNVector3(-19.55,44.79,2284)
 node.geometry?.firstMaterial?.isDoubleSided = true
 print("Loaded node, with url: ", file.absoluteString)
 count += 1
 }catch{
 print("Sorry")
 }
 ...
 // ###### ALERTS ######
 let alert = UIAlertController(title: "Captura de pantalla", message: "La imagen se ha guardado en el carrete de fotos", preferredStyle: UIAlertControllerStyle.alert)
